...
/**
 * The first user list is server-rendered (see page.tsx) with the acting
 * admin's privileges already resolved; searching/filtering and every account
 * action keep the exact server-side authorization they always had.
 */
export default function AdminUsersClient({
  ...
  initialIsSuper: boolean;
  initialError: string | null;
}) {
  const [users, setUsers] = useState<Profile[]>(initialUsers);
  const [loading, setLoading] = useState(false);
  const [isSuper, setIsSuper] = useState(initialIsSuper);
  ...
  useEffect(() => {
    // The initial list was already rendered by the server — only reload when
    // a search/filter actually changes.
    if (!skippedInitialLoad.current) { skippedInitialLoad.current = true; return; }
    load();
  }, [load]);
