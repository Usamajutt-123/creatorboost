...
export default function AdminAnnouncementsClient({
  ...
  initialRecipientCount: number | null;
}) {
  ...
  const [recipientCount, setRecipientCount] = useState<number | null>(initialRecipientCount);
  const [countLoading, setCountLoading] = useState(false);
  const skippedInitialCount = useRef(false);
  // The history is server-rendered (see page.tsx): the composer no longer
  // waits for a post-hydration server-action round-trip on every visit.
  ...
  useEffect(() => {
    if (!initialHistoryError) return;
    toast.error(initialHistoryError);
  }, [initialHistoryError]);
  ...
  useEffect(() => {
    // The initial recipient count was already rendered by the server — only
    // recalculate when the audience or selection actually changes. If the
    // server could not resolve it, fall back to the previous on-mount fetch.
    if (!skippedInitialCount.current) {
      skippedInitialCount.current = true;
      if (initialRecipientCount != null) return;
    }
