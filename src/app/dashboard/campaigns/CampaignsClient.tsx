...
/**
 * The campaign list is server-rendered (see page.tsx): `initial` arrives with
 * the first paint, so the cards no longer wait on a post-hydration
 * auth.getUser() + campaigns fetch. After any mutation the list is refreshed
 * with the same RLS-scoped query it always used.
 */
export default function CampaignsClient({ initial, userId, unreadCount }: { initial: Campaign[]; userId: string; unreadCount: number }) {
  ...
  return (
    <>
      <DashboardTopbar title="My Campaigns" subtitle="Manage all your unlock campaigns" userId={userId} unreadCount={unreadCount} />
