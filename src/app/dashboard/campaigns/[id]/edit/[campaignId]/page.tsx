...
import { Save, Send, Upload, X, Image as ImageIcon, Check, ArrowLeft, Loader2 } from 'lucide-react';
...
const TASK_OPTIONS = [
    { id: 'youtube_subscribe', name: 'YouTube Subscribe', icon: 'â–¶ï¸' },
    { id: 'youtube_like', name: 'YouTube Like', icon: 'ðŸ‘' },
    { id: 'youtube_comment', name: 'YouTube Comment', icon: 'ðŸ’¬' },
    { id: 'watch_video', name: 'YouTube Watch', icon: 'ðŸŽ¬' },
    { id: 'telegram_join', name: 'Telegram Join', icon: 'âœˆï¸' },
    { id: 'discord_join', name: 'Discord Join', icon: 'ðŸŽ®' },
    { id: 'instagram_follow', name: 'Instagram Follow', icon: 'ðŸ“·' },
    { id: 'tiktok_follow', name: 'TikTok Follow', icon: 'ðŸŽµ' },
    { id: 'facebook_follow', name: 'Facebook Follow', icon: 'ðŸ“˜' },
    { id: 'twitter_follow', name: 'X (Twitter) Follow', icon: 'ðŸ¦' },
    { id: 'website_visit', name: 'Website Visit', icon: 'ðŸŒ' },
    { id: 'file_download', name: 'Download App', icon: 'ðŸ“¥' },
    { id: 'custom', name: 'Custom Task', icon: 'âš™ï¸' },
];
...
export default function EditCampaignPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const taskMeta = (data.task_metadata as any) || {};
            setSelectedTasks(tasks.map(t => ({
                id: t,
                title: taskMeta[t]?.title,
                url: taskMeta[t]?.url,
            })));
    ...
    const toggleTask = (taskId: string) => {
        ...
        if (!selectedTasks.find(t => t.id === taskId)) {
            setSelectedTasks([...selectedTasks, { id: taskId }]);
        }
    };
                ...
                <div className="max-w-4xl space-y-6">
                    ...
                    <div className="glass-strong rounded-2xl p-6 space-y-6">
                        <div>
                            ...
                            <div className="grid sm:grid-cols-2 gap-4">
                                ...
                                <div className="sm:col-span-2">
                                    ...
                                    <input value={form.destination_url} onChange={e => setForm({ ...form, destination_url: e.target.value })} className="input-field" />
                                </div>
                            </div>
                        </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <span className={`text-xs text-gray-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`}>â–¾</span>
                                            </button>
                                            {isExpanded && (
                                                <div className="px-3 pb-3 pt-1 border-t border-white/5 mt-1 space-y-2">
                                                    {t.id === 'custom' ? (
                                                        <>
                                                            <input
                                                                ...
                                                                placeholder="Task title (e.g. Visit our website)"
                                                            />
                                                            <input
                                                                value={taskData?.url || ''}
                                                    ...
                                                    <button type="button" onClick={(e) => { e.stopPropagation(); removeTask(t.id); }} className="text-xs text-red-400 hover:text-red-300">
                                                        Remove task
                                                    </button>
                                                </div>
                                            )}
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-end gap-3 pt-4 border-t border-white/5">
                            <Link href="/dashboard/campaigns" className="btn-ghost px-5 py-2.5 rounded-xl text-sm flex items-center justify-center">
                                Cancel
                            </Link>
