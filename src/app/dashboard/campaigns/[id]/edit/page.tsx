...
export default function EditCampaignPage({ params }: { params: { id: string } }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const result = await updateCampaignAction(params.id, {
      ...
      <div className="p-4 sm:p-6"><div className="max-w-4xl space-y-5">
        ...
        <div className="glass-strong rounded-2xl p-5 sm:p-6 space-y-7">
          <section><h2 className="font-semibold mb-4">Basic information</h2><div className="grid sm:grid-cols-2 gap-4">
            ...
            <label className="text-xs font-medium text-gray-300 block">Category<select value={form.category} onChange={event => setForm({ ...form, category: event.target.value as (typeof CATEGORIES)[number] })} className="input-field mt-1.5">{CATEGORIES.map(category => <option key={category} value={category}>{category.replace(/_/g, ' ')}</option>)}</select></label>
          ...
          <section><h2 className="font-semibold mb-4">Publishing settings</h2><div className="grid sm:grid-cols-2 gap-4"><label className="text-xs font-medium text-gray-300 block">Status<select value={form.status} onChange={event => setForm({ ...form, status: event.target.value as typeof form.status })} className="input-field mt-1.5"><option value="active">Active</option><option value="paused">Paused</option><option value="draft">Draft</option></select></label><label className="text-xs font-medium text-gray-300 block">Expiry date (optional)<input type="date" value={form.expiresAt} onChange={event => setForm({ ...form, expiresAt: event.target.value })} className="input-field mt-1.5" /></label></div></section>
