...
import { loadReferralDashboardAction, type ReferralDashboardData } from '@/lib/referral-actions';
...
export default function ReferralsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const result = await loadReferralDashboardAction();
  ...
  const referralLink = data ? `${window.location.origin}/signup?ref=${data.referralCode}` : '';
  ...
  return <>
    <DashboardTopbar title="Referral Program" subtitle={data ? `Earn ${data.commissionRate}% commission on eligible referred-creator earnings` : 'Loading your referral data'} />
